...
namespace Stelark.Analyzers
{
    public class CertificateAnalyzer
    {
        ...
        public async Task AnalyzeESC1CertificatesAsync()
        {
            ...
            // Progress tracking complete (silent mode)
        ...
        private async Task<int> ProcessCertificatesInBatchesAsync(List<Certificate> intenseCerts)
        {
            ...
            await foreach (var line in StreamCertutilViewAsync())
            {
                if (RegexHelper.IsRowStart(line))
                {
                    if (currentBlock.Count > 0)
                    {
                        ...
                        // PERFORMANCE OPTIMIZATION: Dynamic memory-aware batch processing
            ...
            // Fast detection optimization: {skipPercentage:F1}% parsing avoided ({totalSkipped:N0} of {totalCertificatesProcessed:N0}) - logged only
            ...
            string? vulnerabilityType)
        {
            ...
            // Log performance improvements for this template (to logs only)
            if (totalProcessed > 0)
            {
        ...
        private List<Certificate> FilterCertificatesByDate(List<Certificate> certificates)
        {
            ...
            {
                // No date filtering configured - return all certificates
                return certificates;
            }
            ...
            foreach (var cert in certificates)
            {
                ...
                {
                    // Include certificate if it was submitted on or after the start date
                ...
                {
                    // If we can't parse the date, include the certificate to be safe
                    filteredCerts.Add(cert);
        ...
        private bool TryParseCertificateDate(string dateString, out DateTime parsedDate)
        {
            ...
            // Common formats used by certutil:
            // "12/25/2023 10:30 AM"
            // "2023-12-25 10:30:00"
