...
namespace Stelark.Services
{
    public static class MemoryManager
    {
        ...
        public static void ForceGarbageCollection(long maxMemoryMB)
        {
            // Guard against invalid limits; fall back to system estimate if needed
            if (maxMemoryMB <= 0)
            {
                maxMemoryMB = GetTotalSystemMemoryMB();
            }

            // More efficient garbage collection approach
            if (!IsMemoryPressureHigh(maxMemoryMB))
                return;
