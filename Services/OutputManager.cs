...
<head>
    ...
    <style>
        * {{
        ...
        .certificates-table .certificate-row {{
            cursor: pointer;
            transition: background-color 0.2s;
        }}

        .certificates-table .certificate-row:hover {{
...
<body>
    <div class=""container"">
        <div class=""header"">
            <h1>Stelark ADCS Security Report</h1>
            <div class=""subtitle"">
    ...
    <script>
        ...
        function filterTable(tableId, searchValue) {{
            ...
            for (let i = 1; i < rows.length; i++) {{
                // Skip detail rows
                if (rows[i].classList.contains('certificate-details')) continue;
                ...
                if (nextRow && nextRow.classList.contains('certificate-details')) {{
                    if (found) {{
                        // Show detail row only if it was previously expanded
                        nextRow.style.display = nextRow.classList.contains('hidden') ? 'none' : '';
                    }} else {{
        ...
        // Enhanced certificate detail toggle functionality
        function toggleCertDetails(certId) {{
            const detailRow = document.getElementById(certId + '_details');
            if (!detailRow) return;
        ...
        function exportFilteredResults(tableId, format) {{
            const table = document.getElementById(tableId);
            if (!table) return;

            const visibleRows = Array.from(table.rows).filter(row =>
                row.style.display !== 'none' && !row.classList.contains('certificate-details'));
            ...
            if (format === 'csv') {{
                ...
                a.href = url;
                a.download = 'stelark_filtered_certificates.csv';
                a.click();
        ...
        function generateChart() {{
            ...
            if (values.length === 0 || Math.max(...values) === 0) {{
                ...
                container.innerHTML = '<div style=""text-align: center; padding: 40px; color: #27ae60;""><h3 style=""color: #27ae60; margin: 0;"">No Vulnerabilities Detected</h3><p style=""color: #7f8c8d; margin: 8px 0 0 0;"">Your ADCS infrastructure appears to be secure!</p></div>';
                return;
            }}
                        ...
                        <div class=""detail-panel"">
                            <div class=""forensic-section"">
                                ...
                                <div class=""forensic-grid"">
                                    ...
                                    <div class=""forensic-item"">
                                        <strong>Template OID:</strong> {cert.TemplateOID}
                                    </div>
                                    <div class=""forensic-item"">
