    ...
    public static class Logger
    {
        ...
        public static void LogError(string message, Exception? exception = null)
        {
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    if (exception != null)
                    {
                        logEntry += $"{Environment.NewLine}Exception: {exception.GetType().Name}";
                        logEntry += $"{Environment.NewLine}Message: {exception.Message}";
                        logEntry += $"{Environment.NewLine}StackTrace: {exception.StackTrace}";
                        if (exception.InnerException != null)
                        {
                            logEntry += $"{Environment.NewLine}Inner Exception: {exception.InnerException.GetType().Name}";
                            logEntry += $"{Environment.NewLine}Inner Message: {exception.InnerException.Message}";
                        }
        ...
        public static void LogWarning(string message)
        {
            lock (_lock)
            {
                ...
                try
                {
                    var logEntry = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] WARNING: {message}{Environment.NewLine}";
                    File.AppendAllText(_errorLogPath, logEntry);
                }
                catch (Exception ex)
                {
        ...
        private static void LogSessionStart()
        {
            ...
            lock (_lock)
            {
                try
                {
                    ...
                    var logEntry = $"{separator}{Environment.NewLine}" +
                                   $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] SESSION START: Stelark v1.4 - Memory Optimized{Environment.NewLine}" +
        ...
        private static void LogSessionEnd()
        {
            ...
            lock (_lock)
            {
                try
                {
                    ...
                    var logEntry = $"{separator}{Environment.NewLine}" +
                                   $"[{endTime:yyyy-MM-dd HH:mm:ss}] SESSION END: Stelark scan completed (Duration: {durationText}){Environment.NewLine}" +
