...
namespace Stelark.Core
{
    public class StelarkCore : IDisposable
    {
        ...
        {
            Stelark.Services.ConfigManager.LoadConfig();
        ...
        private void Initialize()
        {
            ...
            try
            {
                ...
                Stelark.Services.ConfigManager.LogConfigDetails();
                Stelark.Services.ConfigManager.ShowConfigWarningIfNeeded();
            ...
            if (!_state.FoundCAServers && !_state.IsLocalCAServer)
            {
                ...
                if (IsLocalCertSvcRunning())
                {
                    _state.IsLocalCAServer = true;
                        _state.LocalCAServerName = Environment.MachineName;
                    _state.AllowIntenseFallback = true;
                    _state.IsOfflineMode = true; // Set offline mode flag to prevent LDAP lookups
        ...
        private async Task RunFullAnalysisAsync(bool intense)
        {
            ...
            if (totalVulnTemplates == 0 && !intense)
            {
                ...
                {
                    ConsoleHelper.WriteWarning("Template analysis skipped - Stelark is not running on a Certificate Authority server.");
                    ConsoleHelper.WriteInfo("Run Stelark directly on a CA host to enumerate templates and analyze issued certificates.");
                    Logger.LogInfo("Template scan skipped because this host is not a CA server; certificate analysis unavailable.");
                }
        ...
        private void LogFinalStatistics()
        {
            ...
            var totalAnalyzedCerts = _state.IntenseScanRun && _state.IntenseModeProcessedCount > 0
                ? _state.IntenseModeProcessedCount
